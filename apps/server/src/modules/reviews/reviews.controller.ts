import type { FastifyReply, FastifyRequest } from 'fastify'
import { createReviewBodySchema } from './reviews.model.js'
import type { ReviewsService } from './reviews.service.js'

export function createReviewsController(service: ReviewsService) {
  return {
    async create(request: FastifyRequest, reply: FastifyReply) {
      const parsed = createReviewBodySchema.safeParse(request.body)
      if (!parsed.success) {
        return reply
          .status(400)
          .send({ success: false, error: 'Invalid input', code: 'VALIDATION' })
      }
      const out = await service.create(request.userId!, parsed.data)
      if (out.kind === 'bad_id') {
        return reply.status(400).send({ success: false, error: 'Invalid id', code: 'VALIDATION' })
      }
      if (out.kind === 'forbidden') {
        return reply.status(403).send({ success: false, error: 'Forbidden', code: 'FORBIDDEN' })
      }
      if (out.kind === 'not_found') {
        return reply.status(404).send({ success: false, error: 'Not found', code: 'NOT_FOUND' })
      }
      if (out.kind === 'conflict') {
        return reply
          .status(409)
          .send({ success: false, error: 'Review already exists', code: 'CONFLICT' })
      }
      return reply.send({ success: true, data: { review: out.review } })
    },
  }
}
