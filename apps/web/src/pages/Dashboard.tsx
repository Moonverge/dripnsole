      ...
      <div className="mb-6 flex gap-2 border-b border-border">
        {(['overview', 'listings', 'social'] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`cursor-pointer border-b-2 px-4 py-3 font-martian text-sm capitalize transition-colors ${tab === t ? 'border-black font-bold text-black' : 'border-transparent text-text-muted hover:text-black'}`}
          >
            {t}
          </button>
        ))}
        ...
        <div>
          {selectedIds.size > 0 && (
            <div className="mb-4 flex items-center gap-3 rounded-xl bg-surface-light px-4 py-3">
              ...
              <button
                ...
                className="cursor-pointer rounded-full bg-brand px-4 py-2 font-martian text-xs text-white transition-colors hover:bg-black"
              >
                Post to FB/IG
              </button>
              <button
          ...
          <div className="grid gap-4">
            {myListings.map((listing) => (
              <div
                key={listing.id}
                className="flex items-center gap-4 rounded-xl border border-border p-4 transition-colors hover:bg-surface-light/50"
              >
                <input
                  type="checkbox"
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateAvailability(listing.id, e.target.value as ListingAvailability)
                  }
      ...
      {tab === 'social' && (
        <div className="rounded-2xl border border-border p-8 text-center">
          ...
          <p className="mb-4 font-martian text-sm text-text-muted">
            Select listings from the Listings tab to post to Facebook and Instagram.
          </p>
          <button
            onClick={() => setTab('listings')}
            className="cursor-pointer rounded-full bg-brand px-6 py-3 font-martian text-sm text-white transition-colors hover:bg-black"
          >
            Go to Listings
          </button>
        </div>
