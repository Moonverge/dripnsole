...
export default function CrossPostModal({ listings, storeHandle, onClose, onPost }: CrossPostModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <h2 className="font-martian text-base font-bold">
            {posted ? 'Posted!' : `Cross-Post ${listings.length > 1 ? `${listings.length} Items` : 'to Social'}`}
          </h2>
        ...
        {posted ? (
          <div className="p-6 text-center">
            ...
            <button onClick={onClose} className="mt-4 cursor-pointer rounded-full bg-black px-6 py-2.5 font-martian text-sm text-white">Done</button>
          </div>
        ) : (
          <div className="p-6">
            {listings.length === 1 && (
              <div className="mb-4 flex items-center gap-3 rounded-xl border border-border p-3">
                <img src={listings[0].photos[0]?.url} alt="" className="h-14 w-14 rounded-lg object-cover" />
                <div>
                  <p className="font-martian text-sm font-medium">{listings[0].title}</p>
                  <p className="font-martian text-xs text-text-muted">₱{listings[0].price.toLocaleString()}</p>
                </div>
              </div>
            )}

            <div className="mb-4">
              <label className="mb-1.5 block font-martian text-xs font-medium text-text-secondary">Caption</label>
              <textarea
                value={caption}
            ...
            <div className="mb-6">
              <label className="mb-2 block font-martian text-xs font-medium text-text-secondary">Post to</label>
            ...
            >
              {posting ? 'Posting...' : `Post to ${platforms.size} Platform${platforms.size !== 1 ? 's' : ''}`}
            </button>
          </div>
        )}
      </div>
